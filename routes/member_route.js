...
// Export members list to Excel (Admin/Super Admin only)
router.get(
  "/export",
  authenticated,
  checkAdminRole,
...
router.get(
  ...
  authenticated,
  checkAdminRole,
  MemberController.getMemberRewardEntries
);

// Update a specific reward entry (Admin/Super Admin only)
router.put(
  "/details/:id/rewards/:rewardId",
  authenticated,
  checkAdminRole,
  MemberController.updateRewardEntry
);

// Individual member CRUD operations
router.get("/:id", authenticated, MemberController.getMemberById);
