...
const addPartnerCriterias = asyncHandler(async (req, res, next) => {
  try {
    ...
    return next(
      new ErrorResponse(`Failed to add partner criteria: ${error.message}`, 500)
    );
  }
});
...
const getPartnerCriteria = asyncHandler(async (req, res, next) => {
  try {
    ...
    if (!criteria) {
      logger.error(`Partner criteria not found with id of ${req.params.id}`);
      return next(
        new ErrorResponse(
...
const updatePartnerCriteria = asyncHandler(async (req, res, next) => {
  try {
    ...
    const updateData = {
      ...
      guidelines:
        guidelines !== undefined ? guidelines : existingCriteria.guidelines,
      director_approval:
...
const getPublishedPartnerCriteria = asyncHandler(async (req, res, next) => {
  try {
    ...
    logger.error(`Error getting published partner criteria: ${error.message}`);
    return next(
      new ErrorResponse(
...
const addManagerCriterias = asyncHandler(async (req, res, next) => {
  try {
    ...
    return next(
      new ErrorResponse(`Failed to add manager criteria: ${error.message}`, 500)
    );
  }
});
...
const getManagerCriteria = asyncHandler(async (req, res, next) => {
  try {
    ...
    if (!criteria) {
      logger.error(`Manager criteria not found with id of ${req.params.id}`);
      return next(
        new ErrorResponse(
...
const getPublishedManagerCriteria = asyncHandler(async (req, res, next) => {
  try {
    const { member_title, role_id } = req.userData;

    const { guidelines, member_job_title } = req.query;
    let isDelivery =
      member_title && member_title.toLowerCase().includes("delivery");
    ...
    if (isAdmin) {
      isDelivery = member_job_title.toLowerCase().includes("delivery");
    }

    const criteria = await Criteria.findPublished(true, isDelivery, guidelines);
    res.status(200).json({
      success: true,
      count: criteria.length,
      data: criteria,
    ...
    logger.error(`Error getting published manager criteria: ${error.message}`);
    return next(
      new ErrorResponse(
...
const getCriteriaByDirectorApproval = asyncHandler(async (req, res, next) => {
  try {
    ...
    const criteria = await Criteria.findByDirectorApproval(status, isManager);
