...
const authenticate = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    if (isAdmin) {
      ...
      const validPassword = await Admin.validatePassword(
        password,
        admin.password
      );
    ...
    if (!result.extensionAttribute2) {
      logger.error(
        `Employee ID (extensionAttribute2) missing in LDAP data for user: ${email}`
      );
      return next(
        new ErrorResponse("User profile incomplete. Missing employee ID.", 400)
      );
    }
