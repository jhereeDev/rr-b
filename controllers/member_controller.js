...
const MemberController = {
	...
	updateMember: asyncHandler(async (req, res, next) => {
		try {
			...
			logger.info(`Updating member ${id} with data: ${JSON.stringify(updateData)}`);
			...
			if (updateData.manager_email) {
				...
				if (updateData.manager_email.includes('@')) {
					logger.info(`Looking up manager by email: ${updateData.manager_email}`);
					const manager = await MemberService.findMemberByEmail(updateData.manager_email);

					if (manager) {
						logger.info(`Found manager by email: ${updateData.manager_email} -> ${manager.member_employee_id}`);
						updateData.member_manager_id = manager.member_employee_id;
					} else {
						// If manager not found by email, use the email as ID directly
						// This handles the case where the field contains an employee ID instead of email
						logger.warn(`Manager not found by email, using value as ID: ${updateData.manager_email}`);
						updateData.member_manager_id = updateData.manager_email;
					}
				} else {
					// If not an email, assume it's an employee ID
					updateData.member_manager_id = updateData.manager_email;
				}

				// Remove the manager_email field as it's not in the database schema
				delete updateData.manager_email;
			}

			// Remove the director_email field (if it exists) as we'll get it automatically
			if (updateData.director_email !== undefined) {
				delete updateData.director_email;
			}

			// Format the update data to match database fields
			...
			return next(new ErrorResponse(`Failed to update member: ${error.message}`, 500));
		}
	}),
	...
	searchMembers: asyncHandler(async (req, res, next) => {
		...
		if (!username) {
			return next(new ErrorResponse('Username search pattern is required', 400));
		}
	...
	hidePopup: asyncHandler(async (req, res, next) => {
		...
		try {
			...
			return next(new ErrorResponse(`Failed to hide popup: ${error.message}`, 500));
		}
	}),
	...
	lookupMember: asyncHandler(async (req, res, next) => {
		...
		try {
			logger.info(`Looking up member with: email=${email}, username=${username}, id=${id}`);
			let member;
			...
			if (email) {
				...
				return next(new ErrorResponse('Email, username, or ID parameter is required', 400));
			}
			...
			if (!result) {
				logger.error(`Member lookup failed, no match found: email=${email}, username=${username}, id=${id}`);
			...
			if (result.member_manager_id) {
				try {
					const managerQuery = await database('members').where('member_employee_id', result.member_manager_id).first();
					...
					logger.error(`Error finding manager for member ${result.member_employee_id}:`, err);
			...
			return next(new ErrorResponse(`Failed to lookup member: ${error.message}`, 500));
		}
	}),
};
